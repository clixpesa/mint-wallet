import { TermsOfService } from "@/features/essentials";
import { AnimatedYStack, Button, PhoneInput, Separator, Stack, Text, TextInput, TouchableArea, View, XStack, YStack } from "@/ui";
...
export default function SignInScreen() {
  ...
  const [phoneNumber, setPhoneNumber] = useState<string | undefined>("+254712345678");
	const [email, setEmail] = useState<string | undefined>("rotten@tomata.com");
    ...
    <View flex={1} bg="$surface1" items="center">
      <AnimatedYStack grow={1} width="95%" items="center" gap="$md" >
        ...
        <YStack gap="$sm"  minW="95%" mt="$md">
          {useEmail ? (
            <TextInput placeholder="user@mail.com" fontSize="$lg" py="$md" rounded="$vl" />
          ) : (
            <PhoneInput />
          )}
          <Button variant="branded" size="lg" loading={isLoading} onPress={onPressContinue}>Continue</Button>
          ...
          <Button 
            ...
            size="lg"
            >
            <Text color="$accent1" variant="buttonLabel1">Sign in with Google</Text>
          </Button>
