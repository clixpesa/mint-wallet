import { ScrollView, Stack, Text, Unicon, UniversalImage, UniversalImageResizeMode, XStack, YStack } from "@/ui"
    ...
    <YStack width="100%" mt="$sm" gap="$sm" >
        ...
        <XStack gap="$sm" mx="$sm">
          ...
          (PopularSavings.map(item => (
            <YStack  key={item.id} bg="$surface1" width="auto" p="$sm" gap="$vs" rounded="$md">
              <UniversalImage
                style={{ image: { borderRadius: iconSize } }}
                fallback={<Unicon address="0x765DE816845861e75B25fCA122bb6898B8B1282a" size={iconSize}/>}
                size={{ width: iconSize, height: iconSize, resizeMode: UniversalImageResizeMode.Cover }}
                uri={item.icon}
              />
              <Stack>
                <Text variant="subHeading2" color="$neutral1">{item.name}</Text>
                <Text variant="body3" color="$neutral2">{item.savers}+ savers</Text>
              </Stack>
              <Text variant="subHeading2" color="$accent1">
                ${item.amount[0].toFixed(2)}
                <Text variant="body4" color="$neutral2">
                  {" "}to ${item.amount[1] >= 10000 ? `${item.amount[1]/1000}k` : item.amount[1]}
                </Text>
              </Text>
            </YStack>))
          ) : 
          (
            PopularGroups.map(item => (<YStack  key={item.id} bg="$surface1" width="auto" p="$sm" gap="$vs" rounded="$md">
              <XStack >
                  <UniversalImage
                    style={{ image: { borderRadius: iconSize } }}
                    fallback={<Unicon address={item.ownerAddress} size={iconSize}/>}
                    size={{ width: iconSize, height: iconSize, resizeMode: UniversalImageResizeMode.Cover }}
                    uri={item.ownerIcon}
                  />
              ...
              <YStack gap="$3xs">
                <Text variant="subHeading2" color="$neutral1">{item.name}</Text>
                <Text variant="body3" color="$neutral2">{item.members} members</Text>
              </YStack>
              <Stack>
                <Text variant="subHeading2" color="$accent1">{`${item.target.currency === "USD" ? "$" : "Ksh "}${item.target.amount}`}</Text>
                <Text variant="body4" color="$neutral2">Ends on {item.endDate}</Text>
              </Stack>
            </YStack>))
          )
          }
